//
//  RiskFormViewController.swift
//  Sample
//
//  Created by Amani on 20/11/2025.
//
...
class RiskFormViewController: UIViewController {
    ...
    @IBAction func confirmButtonAction(_ sender: Any) {
        ...
        if check.0 {
            if let dataArray {
                ...
                GatetoPayOnboarding.riskForm.updateRiskData(riskFields: sectionsArray ?? [])
            }
        } else {
            let array = check.1
            if !array.isEmpty {
...
extension RiskFormViewController: UITableViewDelegate, UITableViewDataSource {
...
extension RiskFormViewController: KYCChoiceDelegate {
...
extension RiskFormViewController: UITextFieldDelegate {
...
extension RiskFormViewController: UIDocumentPickerDelegate {
...
extension RiskFormViewController: UIImagePickerControllerDelegate, UINavigationControllerDelegate {
...
extension RiskFormViewController: UITextViewDelegate {
...
extension RiskFormViewController: GatetoPayOnboardingRiskFormDelegate {
    ...
    func didUpdateRiskFormSuccessfully(riskLevel: Int?) {
        ...
        if let sections = sectionsArray {
            ...
            let nationality = allFields.first { $0.fieldLabel == "Nationality" }?.value?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
            let residency = allFields.first { $0.fieldLabel == "Country of Residency"  || $0.fieldLabel ==  "Country Of Residency"}?.value?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""

            print("Nationality = \(nationality)")
            print("Residency = \(residency)")

            // Prevent API call if values are empty
            guard !nationality.isEmpty, !residency.isEmpty else {
                print("Skipping API call — nationality or residency is empty")
                return
            }
...
extension RiskFormViewController: GatetoPayOnboardingCSPDDelegate {
    func cspdTypesReceived(types: [CSPDType]){
        if let vc = storyboard?.instantiateViewController(withIdentifier: "CSPDListViewController") as? CSPDListViewController {
            vc.cspdList = types
